...
// Correction de la condition de vérification de session
if (strlen($_SESSION['vpmsaid']) == 0) {
    header('location:logout.php');
    exit(); // Ajout d'exit() après header
} else {
    if(isset($_POST['submit'])) {
        ...
        $query = mysqli_query($con, "select ID from tbladmin where ID='$adminid' and motdepasse='$cpassword'");
        $row = mysqli_fetch_array($query);
        
        if($row > 0) {
            $ret = mysqli_query($con, "update tbladmin set motdepasse='$newpassword' where ID='$adminid'");
...
<head>
    <title>Green park - Changer le mot de passe</title>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body card-block">
                            <form action="" method="post" enctype="multipart/form-data" class="form-horizontal" name="changepassword" onsubmit="return checkpass();">
                                ...
                                <div class="row form-group">
                                    <div class="col col-md-3">
                                        <label for="confirmpassword" class="form-control-label">Confirmer le mot de passe</label>
                                    </div>
                                    <div class="col-12 col-md-9">
                                        <input type="password" 
                                               id="confirmpassword" 
                                ...
                                <p style="text-align: center;">
                                    <button type="submit" class="btn btn-primary btn-sm" name="submit">Modifier</button>
                                </p>
                            </form>
                        </div>
                    </div>
